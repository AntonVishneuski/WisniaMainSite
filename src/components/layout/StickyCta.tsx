...
export function StickyCta({ settings }: Props) {
  ...
  return (
    <div
      className="sm:hidden fixed bottom-0 inset-x-0 z-[90] flex gap-[10px] px-4 pt-3 pb-[calc(12px+env(safe-area-inset-bottom))]"
      style={{
        background: 'rgba(253,250,247,0.92)',
        backdropFilter: 'blur(14px)',
        WebkitBackdropFilter: 'blur(14px)',
        borderTop: '1px solid rgba(201,149,108,0.35)',
        boxShadow: '0 -8px 24px rgba(110,18,44,0.08)',
      }}
    >
      {/* Booksy – takes flex-1 */}
      <CtaLink
        method="booksy"
        href={booksyHref}
        className="flex-1 flex items-center justify-center gap-2 py-[14px] px-4 rounded-full bg-cherry text-cream text-[15.5px] font-medium border border-transparent shadow-[0_2px_8px_rgba(110,18,44,0.06)] transition-all duration-200 hover:bg-cherry-deep"
      >
        {t('cta.booksy')}
      </CtaLink>
