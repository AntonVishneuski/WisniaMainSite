...
// - Media (images + hero video): served from Vercel Blob *.public.blob.vercel-storage.com;
//   admin uploads (clientUploads:true in payload.config.ts) PUT the file bytes straight from
//   the browser to https://vercel.com/api/blob (the @vercel/blob client SDK's fixed upload
//   endpoint — distinct from the public read/serving domain above)
...
const csp = [
  ...
  "font-src 'self' data:",
  `connect-src 'self'${isDev ? ' ws:' : ''} https://*.public.blob.vercel-storage.com https://vercel.com https://www.googletagmanager.com https://*.google-analytics.com https://stats.g.doubleclick.net https://www.facebook.com https://connect.facebook.net https://*.sentry.io`,
